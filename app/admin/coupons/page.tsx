...
const initialForm = {
  ...
  minimum_order_amount: "", expires_at: "",
};
...
export default function CouponsPage() {
  ...
  const loadCoupons = useCallback(async () => {
    const { data, error } = await supabase.from("coupons")
      .select("id, code, discount_percent, maximum_discount, minimum_order_amount, is_active, expires_at")
  ...
  return <main className="mx-auto max-w-6xl p-5 sm:p-8">
    ...
    <p className="mt-2 text-sm text-gray-600">Create percentage discounts with an optional maximum discount amount.</p>
    <form onSubmit={createCoupon} className="mt-7 grid gap-4 rounded-3xl border border-gray-200 bg-white p-5 shadow-sm sm:grid-cols-2 lg:grid-cols-5">
      ...
      <button disabled={saving} className="rounded-xl bg-green-700 px-4 py-3 text-sm font-extrabold text-white hover:bg-green-800 disabled:bg-gray-400 sm:col-span-2 lg:col-span-5">{saving ? "Saving…" : "Create coupon"}</button>
    </form>
    <div className="mt-7 overflow-hidden rounded-3xl border border-gray-200 bg-white">
      {coupons.length === 0 ? <p className="p-6 text-sm text-gray-600">No coupons yet.</p> : coupons.map((coupon) => <div key={coupon.id} className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 p-5 last:border-0"><div><p className="font-mono text-lg font-extrabold text-gray-900">{coupon.code}</p><p className="mt-1 text-sm text-gray-600">{coupon.discount_percent}% · max {coupon.maximum_discount ?? "—"} SYP · minimum {coupon.minimum_order_amount || 0} SYP</p></div><div className="flex items-center gap-3"><span className={`rounded-full px-3 py-1 text-xs font-extrabold ${coupon.is_active ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}>{coupon.is_active ? "Active" : "Inactive"}</span><button onClick={() => void toggleCoupon(coupon)} className="rounded-xl border border-gray-300 px-3 py-2 text-xs font-extrabold text-gray-700">{coupon.is_active ? "Disable" : "Enable"}</button></div></div>)}
    </div>
  </main>;
}
