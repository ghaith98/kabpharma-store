      ...
      <section className="mx-auto max-w-[1200px]">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {brands.map((brand) => {
            ...
            return <Link key={brand.id} href={isKabPharma ? "/" : `/brands/${brand.slug}`} className="group overflow-hidden rounded-[1.5rem] border border-[#e2e9e4] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
              ...
              <div className="p-6"><h2 className="text-xl font-extrabold text-[#142019]">{title}</h2>{description && <p className="mt-4 text-sm leading-6 text-[#647168]">{description}</p>}<span className="mt-5 inline-flex text-sm font-extrabold text-[#0a583b]">{isKabPharma ? (isArabic ? "زيارة المتجر ←" : "Visit main shop →") : (isArabic ? "اكتشف المجموعة ←" : "Explore collection →")}</span></div>
            </Link>;
          })}
        </div>
      </section>
